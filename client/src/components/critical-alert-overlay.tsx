 ...
 * Top-center banner, one alert at a time, auto-dismiss, prev/next navigation.
 * Notification beep only — TTS removed.
 */

import { useEffect, useState, useRef, useCallback } from "react";
import { X, ChevronLeft, ChevronRight, AlertTriangle, Zap } from "lucide-react";
import type { Alert } from "@shared/schema";
import { soundVerifiedCritical, soundVerifiedHigh, isMuted } from "@/lib/sounds";

const SEV_META: Record<string, { color: string; bg: string; border: string; label: string }> = {
  critical: { color: "#FF1A3E", bg: "rgba(255,26,62,0.08)",   border: "rgba(255,26,62,0.30)",  label: "CRITICAL" },
  high:     { color: "#FFB800", bg: "rgba(255,184,0,0.07)",   border: "rgba(255,184,0,0.25)",  label: "HIGH"     },
  medium:   { color: "#00C8D4", bg: "rgba(0,200,212,0.06)",   border: "rgba(0,200,212,0.20)",  label: "MEDIUM"   },
  low:      { color: "#555555", bg: "rgba(100,100,100,0.04)", border: "rgba(100,100,100,0.15)", label: "LOW"     },
};
...
interface QueuedAlert { id: number; alert: Alert; }
...
export function CriticalAlertOverlay({ alerts }: Props) {
  ...
  const dismissAll = useCallback(() => {
    clearTimer();
    setExiting(true);
    setTimeout(() => { setQueue([]); setIdx(0); setVisible(false); setExiting(false); }, 350);
  }, []);

  const dismiss = useCallback((removeId?: number) => {
    clearTimer();
    setQueue(prev => {
      const next = removeId !== undefined ? prev.filter(q => q.id !== removeId) : prev.slice(1);
      if (next.length === 0) {
        setExiting(true);
  ...
  useEffect(() => {
    ...
    // Play notification beep for first critical alert
    if (!isMuted()) {
      const firstCrit = newAlerts.find(a => a.severity === "critical");
      if (firstCrit) soundVerifiedCritical();
      else soundVerifiedHigh();
    }
  }, [alerts]);
  ...
  return (
    <div
      ...
      style={{ top: 48, left: "50%", transform: "translateX(-50%)", width: "min(560px, calc(100vw - 2rem))" }}
    >
      <div
        ...
        >
          ...
          <div className="px-4 py-3 flex items-center gap-3">
            ...
            <div className="shrink-0 flex items-center justify-center w-9 h-9 rounded-lg"
              ...
              {isCritical
                ? <AlertTriangle className="w-4.5 h-4.5" style={{ color: meta.color }} />
                : <Zap className="w-4.5 h-4.5" style={{ color: meta.color }} />
              }
            </div>
                  ...
                  <span className="text-[8px] font-mono text-white/30 flex items-center gap-1">
                    {alert.countryCode && alert.countryCode.length === 2 && (
                      <img
                        ...
                        alt="" className="w-3.5 h-2.5 rounded-sm opacity-60"
                      />
                    )}
                    {alert.country}
                  </span>
