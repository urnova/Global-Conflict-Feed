...
// ── Error boundary — protects the whole dashboard if WebGL isn't available ────
class GlobeErrorBoundary extends Component<{ children: ReactNode }, { error: boolean }> {
  state = { error: false };
    ...
    if (this.state.error) {
      return (
        <div className="absolute inset-0 bg-black flex flex-col items-center justify-center text-center p-8">
          ...
          <div className="text-primary font-bold text-sm tracking-widest uppercase mb-2">Globe 3D indisponible</div>
          <div className="text-muted-foreground text-xs font-mono max-w-xs">
            WebGL non supporté dans cet environnement. Le reste du dashboard reste opérationnel.
          </div>
        </div>
      );
    }
...
const TYPE_ICONS: Record<string, string> = {
  ...
  wildfire: '🔥', avalanche: '⛰️', landslide: '⛰️',
  // MÉTÉO
  hurricane: '🌀', cyclone: '🌀', tornado: '🌪️', storm: '⛈️',
  heatwave: '🌡️', 'cold-snap': '❄️', drought: '☀️',
  // SANTÉ
  pandemic: '🦠', epidemic: '🦠', outbreak: '🦠', biological: '🧬',
  // INFORMATIONS
...
// Category → color for globe pins
const CAT_PIN_COLORS: Record<string, string> = {
  CATASTROPHE: '#FF6B00',
  MÉTÉO:       '#3B9EFF',
  SANTÉ:       '#00D68F',
  MILITAIRE:   '#FF003C',
